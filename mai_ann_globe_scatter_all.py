...
area1=NetCDFFile('/scratch2/scratchdirs/tslin2/plot/globalcrop/data/Hisfaofert_plot.nc','r')
maifert=area1.variables['maize_fert'][:]
...
iyield1ynewe = dat.variables['g_ET'][0:105,0,:,:]
...
iyield2ynewe = dat2.variables['g_ET'][0:105,0,:,:]
...
iyield3ynewe = dat3.variables['g_ET'][0:105,0,:,:]
...
iyield4ynewe = dat4.variables['g_ET'][0:105,0,:,:]
...
iyield5ynewe = dat5.variables['g_ET'][0:105,0,:,:]


dat6=NetCDFFile('/scratch2/scratchdirs/tslin2/isam/cheyenne/yieldout/isamhistorical_cru/heat/fertfao/fixedirr_new1/evp/maizetemp_historical_co2_rf_nofert_0.5x0.5_evp.nc','r')
iyield6ye = N.average(dat6.variables['g_ET'][95:105,0,:,:],axis=0)
iyield6ynewe = dat6.variables['g_ET'][0:105,0,:,:]
...
maifert,lonisam2=shiftgrid(0.5,maifert,lonisam1,start=True)
...
fert=N.average(maifert,weights=maizeto2,axis=(1,2))
fertr=N.zeros(105)
for i in range(0,105):
	fertr[i]=(fert[i]-fert[0])/fert[104]*100
##yield (t/ha)
...
##ET relative change 
etfinalnew=(allynewe-allyinnewe)/allynewe*100
etfinalnnew=(allynewe-allynnewe)/allynewe*100
etfinalinew=(allynewe-allyinewe)/allynewe*100
etfinalclinew=(allynewe-allyclinewe)/allynewe*100
etfinalcnew=(allynewe-allycnewe)/allynewe*100

etfnoi=(allyinewe-allyinnewe)/allyinewe*100
...
ffnoi=(allyinew-allyinnew)/allyinew*100

##water use efficiency relative change 
wfinalnew=(allynewew-allyinnewew)/allynewew*100
wfinalnnew=(allynewew-allynnewew)/allynewew*100
wfinalinew=(allynewew-allyinewew)/allynewew*100
wfinalclinew=(allynewew-allyclinewew)/allynewew*100
wfinalcnew=(allynewew-allycnewew)/allynewew*100

wffnoi=(allyinewew-allyinnewew)/allyinewew*100
...
ax.scatter(fertr,finalnnew,c='blue',alpha=1,label='Irrigated')
ax.scatter(fertr,ffnoi,c='red',alpha=1,label='Rainfed')
plt.xlabel('Relative change in NF (%)',fontsize=16)
plt.ylabel('Relative change in yield (%)',fontsize=16)
plt.tick_params(axis='both',labelsize=16)
ax.legend(loc='2')

slope, intercept, r_value, p_value, std_err = stats.linregress(fertr,finalnnew)
...
slope, intercept, r_value, p_value, std_err = stats.linregress(fertr,ffnoi)
line = slope*fertr+intercept
ax.plot(fertr,line,linewidth=2.0,color='red')
rr=r_value**2
ax.annotate('y = {:05.3f} x'.format(slope), xy=(0.54, 0.20),xycoords='axes fraction',fontsize=18,color='red')
ax.annotate('R$^2$ = {:04.2f}'.format(rr), xy=(0.51, 0.12),xycoords='axes fraction',fontsize=18,color='red')
...
ax.scatter(fertr,wfinalnnew,c='blue',alpha=1)
plt.xlabel('Relative change in NF (%)',fontsize=16)
plt.ylabel('Relative change in WUE (%)',fontsize=16)
plt.tick_params(axis='both',labelsize=16)

slope, intercept, r_value, p_value, std_err = stats.linregress(fertr,wfinalnnew)
line = slope*fertr+intercept
ax.plot(fertr,line,linewidth=2.0)
rr=r_value**2
ax.annotate('y = {:05.3f} x'.format(slope), xy=(0.14, 0.70),xycoords='axes fraction',fontsize=18,color='blue')
...
ax.scatter(fertr,wffnoi,c='red',alpha=1)
slope, intercept, r_value, p_value, std_err = stats.linregress(fertr,wffnoi)
line = slope*fertr+intercept
ax.plot(fertr,line,linewidth=2.0,color='red')
rr=r_value**2
ax.annotate('y = {:05.3f} x'.format(slope), xy=(0.54, 0.20),xycoords='axes fraction',fontsize=18,color='red')
...
ax.scatter(fertr,etfinalnnew,c='black',alpha=1)
plt.xlabel('Relative change in NF (%)',fontsize=16)
plt.ylabel('Relative change in ET (%)',fontsize=16)
plt.tick_params(axis='both',labelsize=16)

slope, intercept, r_value, p_value, std_err = stats.linregress(fertr,etfinalnnew)
line = slope*fertr+intercept
ax.plot(fertr,line,linewidth=2.0)
rr=r_value**2
ax.annotate('y = {:05.3f} x'.format(slope), xy=(0.14, 0.70),xycoords='axes fraction',fontsize=18,color='blue')
...
ax.scatter(fertr,etfnoi,c='red',alpha=1)
slope, intercept, r_value, p_value, std_err = stats.linregress(fertr,etfnoi)
line = slope*fertr+intercept
ax.plot(fertr,line,linewidth=2.0,color='red')
rr=r_value**2
ax.annotate('y = {:05.3f} x'.format(slope), xy=(0.54, 0.20),xycoords='axes fraction',fontsize=18,color='red')
...
plt.savefig('maihis_wue_1902_2005_sall.png')
plt.show()
